...
# 认证方式二：直接传入 Session ID（优先级更高，设置后忽略用户名密码）
...
Query API 默认注入 `limit=50`，防止一次拉取全量数据撑满模型上下文窗口。响应超过 64 KiB 时会自动裁剪 `inventories` 列表，保证返回合法 JSON。
...
- 显式传入 `limit` 时不会被覆盖。
- 裁剪发生时响应中会包含 `_truncation` 字段，提示使用 `limit`/`start` 翻页或通过 `fields` 精简返回字段。
...
> 说明：也兼容 `FASTMCP_HOST`、`FASTMCP_PORT` 和 `FASTMCP_MOUNT_PATH`（FastMCP 原生环境变量）。
...
- `namespace` (`str`，可选)：按命名空间过滤，支持模糊匹配，如 `vm` 或 `host`
- `limit` (`int`，默认 `20`)：最多返回数量
- `match_mode` (`str`，默认 `or`)：关键词匹配模式，可选 `and` 或 `or`
- `prefer_namespaces` (`list[str]`，可选)：优先排序的命名空间列表，默认 `["ZStack/VM", "ZStack/Host"]`

> 💡 不确定 namespace 时可先不传，返回结果会带 namespace 值供选择。
>
...
`series_count` 为不同 label 组合数量；不传 `labels` 时可能返回多组序列。建议通过缩短时间范围、增大 `period` 或增加 `labels` 过滤来避免输出过大。

### 6. `get_metric_summary`

获取监控指标的聚合 Top-N（按 `label_key` 分组）。

**参数：**

- `namespace` (`str`)：命名空间
- `metric_name` (`str`)：指标名称
- `label_key` (`str`)：标签键，如 `VMUuid` 或 `HostUuid`
- `metric_names` (`list[str]`，可选)：多指标合并，如流入和流出指标
- `start_time` (`str | int`，可选)：开始时间（ISO 或秒级时间戳）
- `end_time` (`str | int`，可选)：结束时间（ISO 或秒级时间戳）
- `period` (`int`，默认 `60`)：采样周期（秒）
- `aggregate` (`str`，默认 `max`)：单指标聚合方式，可选 `max`、`avg`、`sum` 或 `min`
- `combine` (`str`，默认 `sum`)：多指标合并方式，可选 `sum`、`avg`、`max` 或 `min`
- `threshold_op` (`str`，可选)：阈值比较符，可选 `>`、`>=`、`<`、`<=`、`==` 或 `!=`
- `threshold_value` (`number`，可选)：阈值数值
- `top_n` (`int`，默认 `10`)：返回条数
- `resolve_resource` (`str`，可选)：`vm` 或 `host`，用于解析资源名称
